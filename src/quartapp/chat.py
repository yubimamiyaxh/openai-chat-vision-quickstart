...
async def call_model_on_image(image_base64, user_message):
    ...
    section_prompt = "The uploaded file is scanned medical documents of one or more medical patients. Identify the following information for each patient if it is in the documents: their full legal name, date of birth, sex, living address, email address, phone number, primary insurance name, primary insurance type, primary insurance Member ID number, primary insurance Group ID number, secondary insurance name, secondary insurance type, secondary insurance Member ID number, secondary insurance Group ID number, CPT codes, and ICD codes. The primary insurance may also be referred to as the main insurance or first insurance in these documents. There are two possible insurance types, Medicare and Commercial, where Commercial encompassses all insurances that are not Medicare. When a patient has both a commercial insurance and a Medicare only insurance, the Medicare insurance is the primary plan and the commercial insurance is the secondary plan. The Member ID number and the Group ID number consists of any combination of uppercase letters and numerical digits. In the returned information, the phone number should be returned as 10 digits with no dashes, parentheses, or spaces. In the returned information, the sex should be represented as either F for female or M for male. In the returned information, all of the commas should be removed from the living address. If there are multiple phone numbers listed for the patient, the returned information should provide their cell phone number. In the returned information, the date of birth should be written in MM/DD/YYYY format where the month, day, and year are represented numerically. A CPT code is a numerical five-digit code that represents medical services and procedures. If there are multiple CPT codes, they should be separated by the word \'and\' in the returned information. An ICD code is an alphanumeric code that contains up to seven characters that represents a type of disease or health condition in a patient. If there are multiple ICD codes, they should be separated by the word \'and\' in the returned information. For every piece of returned information, return it in a key-value pair separated by a colon where the key is the patient\'s full legal name and the value is the relevant returned information. All of the key-value pairs should be returned as a comma separated list."
...
async def process_pdf():
    ...
    # YUBI: make sure that all ' characters are formatted correctly
    final_prompt="Prompt: This is a comma separated list of key-value pairs containing relevant information on one or more medical patients. Every key is a patient\'s full name and the associated value is one of the following: their full legal name, date of birth, sex, living address, email address, phone number, primary insurance name, primary insurance type, primary insurance Member ID number, primary insurance Group ID number, secondary insurance name, secondary insurance type, secondary insurance Member ID number, secondary insurance Group ID number, CPT codes, and ICD codes. There may be keys with similar full names that can be reasonably assumed to belong to the same patient. For example, some full names may include a middle initial, middle name, or maiden name. If there are similar keys, replace all of the sets of keys that are similar to each other with the same, longest full name that is known in each set. Create a table where there is one row per patient, and the columns are each patient\'s full legal name, date of birth, sex, living address, email address, phone number, primary insurance name, primary insurance type, primary insurance Member ID number, primary insurance Group ID number, secondary insurance name, secondary insurance type, secondary insurance Member ID number, secondary insurance Group ID number, CPT codes, and ICD codes. The first row of the table should be the names of the columns only. If there is any missing information, write N/A in that table entry. Return the table as a comma separated list where each column is separated by a comma only, and each row is separated by a semicolon only."
