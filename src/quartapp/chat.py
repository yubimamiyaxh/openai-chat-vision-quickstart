...
async def call_model_on_image(image_base64, user_message, processing_mode):
    ...
    if processing_mode == "payment":
        section_prompt += f"The file is a series of scanned letters that contains Explanation of Benefits (EOB) and associated Payments for medical services. The EOB is from a medical patient\'s health insurance company and analyzes their medical costs in a chart. For every EOB, I want to know the full name of the patient, the amount of money paid by the health insurance company (Amount Paid), and the name of the health insurance company. The Amount Paid is written in the bottom-right most entry in the EOB chart and is displayed as a Total. There are 2 types of payment: Check and Virtual Credit Card. For every payment, I want to know the payer name, receiver name, the amount of money of the payment (Monetary Value), payment type, and page number it is on. The page number is written as \'Page # of {total_pg_count}\' on every page, where # represents the page number. For every check, I want to know the check number. For every virtual credit card, I want to know the credit card number, the CVV code, and the expiration date. Represent the extracted information as one of the attached JSON schemas based on whether it is an EOB or a payment. Return two arrays in a JSON object with the following keys: page_array and objects_array. page_array is an array of all the page numbers containing a check or virtual credit card payment. objects_array is an array of all JSON data instances found. Format each array and your full response as raw JSON only. Do not include any explanation or commentary. Do not wrap the response in markdown backticks."
