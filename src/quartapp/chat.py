...
async def process_pdf():
    ...
    # YUBI: make sure that all ' characters are formatted correctly
    final_prompt="Prompt: This is a comma separated list of key-value pairs containing relevant information on one or more medical patients. Every key is a patient\'s full name and the associated value is one of the following: their full legal name, date of birth, sex, living address, email address, phone number, primary insurance name, primary insurance type, primary insurance Member ID number, or primary insurance Group ID number. Create a table where there is one row per patient and the columns are each patient\'s full legal name, date of birth, sex, living address, email address, phone number, primary insurance name, primary insurance type, primary insurance Member ID number, or primary insurance Group ID number. The first row of the table should be the names of the columns. If there are keys with similar full names that can be reasonably assumed to belong to the same patient, join those values together. For example, some full names may or may not include a middle initial, middle name, or maiden name. If there is any missing information, write N/A in that table entry. Return the table as a comma separated list where each column is separated by a comma only without any extra space after the comma and each row is separated by a semicolon only without any extra space after the semicolon."
